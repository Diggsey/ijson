...
// Specific implementation for extending with IValue - no specialization needed
...
// Specific implementations for IValue (heterogeneous arrays) - no specialization needed
impl From<Vec<IValue>> for IArray {
    fn from(other: Vec<IValue>) -> Self {
        let mut res = IArray::with_capacity(other.len());
        res.extend(other.into_iter());
        res
    }
